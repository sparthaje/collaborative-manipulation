...
Usage:
    from model.dataset import SiameseVLADataset, build_combined_dataset
    dataset = build_combined_dataset("data")           # all datasets
    dataset = SiameseVLADataset("data/some_dataset")   # single dataset
"""
...
class SiameseVLADataset(Dataset):
    ...
    def __init__(
        ...
        # Load action tokenizer (use provided global one, or build from local stats)
...
def build_combined_dataset(
    ...
    Discovers all subdirectories with meta/info.json, loads their ignore
    lists, builds a global ActionTokenizer from all stats.json files,
    and creates one SiameseVLADataset per directory.
...
def get_global_tokenizer(data_dir: str | Path) -> ActionTokenizer:
    """Build a global ActionTokenizer from all datasets under data_dir."""
    data_dir = Path(data_dir)
    dataset_dirs = sorted(
        p for p in data_dir.iterdir()
        if p.is_dir() and (p / "meta" / "info.json").exists()
    ...
    stats_paths = [d / "meta" / "stats.json" for d in dataset_dirs]
    return ActionTokenizer.from_multiple_stats(stats_paths)
